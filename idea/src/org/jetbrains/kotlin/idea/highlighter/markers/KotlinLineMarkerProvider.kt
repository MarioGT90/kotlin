...
internal fun KtDeclaration.findMarkerBoundDeclarations(): List<KtNamedDeclaration> {
    if (this !is KtClass && this !is KtParameter) return emptyList()
    val document = PsiDocumentManager.getInstance(project).getDocument(containingFile)
    return when (this) {
        is KtParameter -> takeIf { hasValOrVar() }?.containingClassOrObject?.primaryConstructorParameters?.filter {
            it !== this && it.hasValOrVar() && document?.areAnchorsOnOneLine(this, it) == true
        }.orEmpty()
        is KtEnumEntry -> containingClassOrObject?.body?.enumEntries?.filter {
            it !== this && document?.areAnchorsOnOneLine(this, it) == true
        }.orEmpty()
        is KtClass -> {
            val result = mutableListOf<KtNamedDeclaration>()
            result += primaryConstructor?.valueParameters?.filter {
                it.hasValOrVar() && document?.areAnchorsOnOneLine(this, it) == true
            }.orEmpty()
            if (this.isEnum()) {
                result += this.body?.enumEntries?.filter {
                    document?.areAnchorsOnOneLine(this, it) == true
                }.orEmpty()
            }
            result
        }
